  ...
  useEffect(() => {
    const fetchServiceData = async () => {
      try {
        ...
        const currentService = allServices.find(s => s.route === '/service/ai-automation');
        if (currentService) {
            ...
            <div className="bg-white p-3 p-md-5 rounded shadow-sm">
              <div className="d-flex align-items-center mb-4 flex-wrap">
                <div className="btn-square bg-primary rounded-circle me-3 ms-3 mb-2">
                  <i className={`fa ${service.icon || 'fa-microchip'} text-white`}></i>
                </div>
              ...
              <div className="text-center mt-5">
                <a href={`/contact?subject=${encodeURIComponent(isAr ? 'طلب خدمة أتمتة العمليات' : 'Process Automation Service Request')}`} className="btn btn-primary py-3 px-5 rounded-pill shadow-sm">
