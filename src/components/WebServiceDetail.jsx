  ...
  useEffect(() => {
    const fetchServiceData = async () => {
      try {
        ...
        const currentService = allServices.find(s => s.route === '/service/web-applications');
        if (currentService) {
            ...
            <div className="bg-white p-3 p-md-5 rounded shadow-sm">
              ...
              <div className="row g-4">
                  ...
                  <div className="col-12 mt-5">
                    ...
                    <div className="row g-4">
                      {results.filter(f => f.section === 'how').map((item, index) => (
                        <div className={`col-md-6 col-lg-4 scroll-reveal ${index % 2 === 0 ? 'from-left' : 'from-right'}`} data-delay={index * 100} key={item.id}>
                ...
                <div className="col-12 mt-5 text-center">
                  <a href={`/contact?subject=${encodeURIComponent(isAr ? 'طلب خدمة تطبيقات الويب' : 'Web Applications Service Request')}`} className="btn btn-primary py-3 px-5 rounded-pill shadow-sm">
